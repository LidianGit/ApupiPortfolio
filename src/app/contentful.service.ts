...
export class ContentfulService {
    ...
    getProjects(query?: object): Promise<EntryCollection<any>> {
        return this.client.getEntries<any>(Object.assign({content_type: 'project'}, query))
            .then((res) => {
                    if (isNullOrUndefined(res.items)) {
                        res.items = [];
                    }
                    if ( res.items.length < 8 ) {
                        res.items = res.items.concat(MOCK_PROJECTS);
                    }
                    return res;
                },
                (err) => {
...
const MOCK = '{' +
    ...
    ' quam nihil molestiae consequatur, vel illum, qui dolorem eum fugiat, quo voluptas nulla pariatur? [33] At vero eos et accusamus et iusto odio dignissimos ducimus, qui blanditiis praesentium voluptatum deleniti ' +
    'atque corrupti, quos dolores et quas molestias excepturi sint, obcaecati cupiditate non provident, similique sunt in culpa, qui officia deserunt mollitia animi, id est laborum et dolorum fuga. Et harum quidem rerum' +
